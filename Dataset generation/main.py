...
if __name__ == '__main__':
    ...
    # Input the number of rows in the dataset
    N = int(input('N = '))
    ...
    while current_amount != N:
        ...
        for i in range(number_of_employees):
            current_job_title = random.choices(list(config.WEIGHTS_FOR_JOB_TITLES.keys()), weights=list(config.WEIGHTS_FOR_JOB_TITLES.values()), k=1)[
                0]
            ...
            dataset[current_amount] = [generate_random_full_name(), generate_random_phone_number(), current_address, current_job_title, salary]
            current_amount += 1
